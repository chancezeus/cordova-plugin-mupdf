...
static int saveDoc(const char *current_path, fz_document *doc)
{
    int written = 0;
    char *tmp;
...
static char* saveDocCopy(char *current_path, fz_document *doc)
{
    char *tmp = tmp_path(current_path);
...
{
    self = [self initWithFilename:filename path:path document:aDoc];
    if (!self)
        return nil;
